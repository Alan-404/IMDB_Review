import re
import numpy as np
from replace import REReplacer

class Tokenizer:
    def __init__(self):
        self.word_index = dict()
        self.index_word = dict()
        self.replacer = REReplacer()
        self.num_words = dict()
        self.index = 1
        self.bag_of_words = []
        self.bag_of_onehot = []
    ...
    def bag2onehot(self):
        for context, target in self.bag_of_words:
            context_onehot = [self.find_one_hot_vector(word) for word in context]
            target_onehot = self.find_one_hot_vector(target)
            self.bag_of_onehot.append((context_onehot, target_onehot))
